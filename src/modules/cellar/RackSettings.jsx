...
export default function RackSettings({ onClose }) {
  const { racks, addRack, renameRack, removeRack, addSlot, renameSlot, removeSlot } = useCellar()
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-gray-800 rounded-t-3xl shadow-2xl sheet-enter max-h-[92vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            <div key={r.id} className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                <button onClick={() => {
                  ...
                  if (l) addSlot(r.id, l)
                }} className="text-xs text-rose-600 font-semibold px-2 py-0.5">+ Fach</button>
              </div>
            </div>
          ))}
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { if (newLabel.trim()) { addRack(newLabel, newEmoji); setNewLabel('') } }}
                ...
                className="btn-primary text-sm px-4 disabled:opacity-40" style={{ backgroundColor: '#e11d48' }}>
                Anlegen
              </button>
            </div>
          </div>
          ...
          <div className="pt-4 pb-4">
            <button onClick={onClose} className="btn-primary w-full" style={{ backgroundColor: '#e11d48' }}>Fertig</button>
          </div>
        </div>
      </div>
    </>
